...
.srch-cli{flex:1;min-width:200px;padding:.5rem 1rem .5rem 2.25rem;border:1.5px solid #eee;border-radius:10px;font-size:.85rem;font-family:'Nunito',sans-serif;outline:none;background:white;}
...
.srch-wrap-cli i{position:absolute;left:10px;top:50%;transform:translateY(-50%);color:#aaa;font-size:.82rem;}
...
.btn-view-book{
    flex:1;padding:.38rem .5rem;background:#1a0a0f;color:white;
    border:none;border-radius:8px;font-size:.76rem;font-weight:700;
    text-decoration:none;display:flex;align-items:center;justify-content:center;
    gap:.3rem;font-family:'Nunito',sans-serif;transition:background .2s;
}
.btn-view-book:hover{background:#3d1020;color:white;}
.btn-wish-book{
    width:34px;height:34px;background:#fff0f2;color:var(--cherry);
...
<div class="filter-bar">
    <div class="srch-wrap-cli">
        ...
        <input type="text" class="srch-cli" id="book-search"
               placeholder="Kërko titull ose autor..." autocomplete="off">
    </div>
    <span style="color:#aaa;font-size:.82rem;">{{ $books->count() }} libra</span>
</div>
...
<div class="books-grid" id="books-container">
    ...
    <div class="book-card"
         ...
         data-author="{{ strtolower($book->author->emri ?? '') }} {{ strtolower($book->author->mbiemri ?? '') }}"
        ...
        <div class="book-body">
            ...
            <div class="book-actions">
                ...
                <form action="{{ route('wishlist.store') }}" method="POST">
                    ...
                    <button type="submit" class="btn-wish-book" title="Shto në listën e dëshirave">
