...
@section('title','Edito Kategorinë')
...
<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:20px;flex-wrap:wrap;gap:10px;">
    <div>
        <h4 style="font-weight:900;margin:0 0 2px;">Edito: {{ $category->emri }}</h4>
        <p style="color:#aaa;margin:0;font-size:.83rem;">Ndrysho të dhënat e kategorisë</p>
    </div>
...
<div class="form-card">
    <form action="{{ route('categories.update', $category->id) }}" method="POST">
        ...
        <div class="form-row">
            ...
            <input type="text" name="emri" class="form-inp"
                   value="{{ old('emri', $category->emri) }}" required>
        </div>

        <div class="form-row">
            <label class="form-lbl">Përshkrimi</label>
            <textarea name="pershkrimi" class="form-ta" rows="3">{{ old('pershkrimi', $category->pershkrimi) }}</textarea>
        </div>
        ...
        <div class="form-row">
            ...
            <select name="kategoria_prind_id" class="form-sel">
                ...
                @foreach($parentCategories as $p)
                    <option value="{{ $p->id }}"
                        {{ old('kategoria_prind_id', $category->kategoria_prind_id) == $p->id ? 'selected' : '' }}>
                        {{ $p->emri }}
                    </option>
                @endforeach
            </select>
